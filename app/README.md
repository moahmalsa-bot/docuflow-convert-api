...
- `page_count`
- `scanned_pages`
- `objects`, each with `page_number`, `object_id`, `object_type`, `text`, `bounding_box`, `font_size`, and `confidence`
...
Applies approved operations to a new PDF and never overwrites the original. `replace_image` operations require `replacement_image_base64`.
