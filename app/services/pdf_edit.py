...
def apply_ai_edit_operations(document_id: str, operations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    with fitz.open(source_pdf) as document:
        for operation in validated:
            ...
            _log_edit_resolution(operation, source_pdf, scratch_pdf)
    ...
    _verify_saved_replacements(scratch_pdf, verification_targets)
...
def _redact_area(page: fitz.Page, rect: fitz.Rect, fill: Color = (1, 1, 1)) -> None:
    page.add_redact_annot(_expanded_rect(rect, page.rect), fill=fill)
    page.apply_redactions()
...
def _insert_replacement_text(page: fitz.Page, rect: fitz.Rect, operation: dict[str, Any]) -> None:
    ...
    baseline = min(rect.y1 - 1, rect.y0 + font_size)
    page.insert_text(
...
def _fit_font_size(page: fitz.Page, rect: fitz.Rect, text: str, starting_size: float) -> float:
    ...
    while size > 4:
        ...
        if text_width <= rect.width and size <= rect.height:
            return size
        size -= 0.5
    return 4


def _expanded_rect(rect: fitz.Rect, page_rect: fitz.Rect, padding: float = 1.5) -> fitz.Rect:
...
def _verify_saved_replacements(saved_pdf: Path, targets: list[dict[str, str]]) -> None:
    if not targets:
        return
    ...
    for target in targets:
        ...
        if original_text and original_text in normalized_extracted:
            saved_pdf.unlink(missing_ok=True)
            raise HTTPException(status_code=422, detail="Edited PDF verification failed: original text is still present")
...
def _log_edit_resolution(operation: dict[str, Any], source_pdf: Path, scratch_pdf: Path) -> None:
    logger.info(
        json.dumps(
            {
