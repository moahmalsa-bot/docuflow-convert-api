...
def apply_ai_edit_operations(document_id: str, operations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    with fitz.open(source_pdf) as document:
        for operation in validated:
            ...
            padded_rect = _expanded_rect(rect, page.rect, padding=2.0)
            before_digest = _render_clip_digest(page, padded_rect)
            _log_edit_resolution(operation, source_pdf, scratch_pdf, extracted_text_before, padded_rect)

            if op_type == "replace_text":
                fallback_warning = None
                try:
                    _redact_area(page, padded_rect, fill=(1, 1, 1), expand=False)
                except Exception:
                    logger.exception("PyMuPDF redaction failed; applying visual replacement fallback")
                    _whiteout(page, padded_rect)
                    fallback_warning = "Visual replacement applied; hidden original text may still exist."
                ...
                verification_targets.append(
                    {
                        ...
                        "redaction_rect": _bbox(padded_rect),
    ...
    warnings.extend(_verify_saved_replacements(scratch_pdf, verification_targets))
...
def _verify_saved_replacements(saved_pdf: Path, targets: list[dict[str, Any]]) -> list[str]:
    if not targets:
        return []
    warnings: list[str] = []
    with fitz.open(saved_pdf) as document:
        extracted_text = "\n".join(page.get_text("text") for page in document)
        per_page_text = {page_number + 1: page.get_text("text") for page_number, page in enumerate(document)}
        after_digests = {
            index: _render_clip_digest(document[int(target["page"]) - 1], _as_rect(target["redaction_rect"]))
    ...
    for index, target in enumerate(targets):
        ...
        visual_changed = bool(target.get("before_digest")) and target.get("before_digest") != after_digests.get(index)
        normalized_page_text = _normalize_text(per_page_text.get(int(target["page"]), ""))
        if new_text and new_text not in normalized_extracted:
            if visual_changed:
                ...
                raise HTTPException(status_code=422, detail="Edited PDF verification failed: replacement text was not found")
        if original_text and original_text in normalized_extracted:
            if visual_changed and (not new_text or new_text in normalized_page_text):
                warnings.append("Original text may remain in hidden PDF extraction layer, but visual text was replaced.")
            else:
                saved_pdf.unlink(missing_ok=True)
                raise HTTPException(status_code=422, detail="Edited PDF verification failed: original text is still present and visual replacement could not be confirmed")
        ...
        json.dumps(
            {
                ...
                "padded_redaction_rect": _bbox(padded_rect),
