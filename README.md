...
Applies approved operations to a new PDF and never overwrites the original. `operation` is the canonical operation-name field. For backward compatibility, `type` is also accepted. `replace_text` is visual replacement by default: DocuFlow draws a white rectangle over the selected object bounding box, inserts `new_text` on top with shrink-to-fit behavior, saves a new version, renders the edited page, and returns success only when the visible page changed. `replace_image` operations require `replacement_image_base64`.

PDF replacement is verified visually, not by internal extracted text. Some PDFs keep stale hidden extraction text even when the visible page is correct. In that case the endpoint returns HTTP 200 with a warning instead of failing the edit.

AI replace text can be planned with instructions such as:

```text
Change OLD TEXT to NEW TEXT
```

DocuFlow searches analyzed native text and OCR text objects for `OLD TEXT`. If one match is found, `/pdf/ai-edit/plan` returns a validated `replace_text` operation. If multiple matches are found, the response asks for confirmation and includes match objects with ready-to-apply operations for each match.
