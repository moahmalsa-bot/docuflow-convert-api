...
Applies approved operations to a new PDF and never overwrites the original. `operation` is the canonical operation-name field. For backward compatibility, `type` is also accepted. `replace_text` uses real PyMuPDF redaction with a small padded rectangle, then inserts `new_text` with shrink-to-fit behavior. `replace_image` operations require `replacement_image_base64`.

PDF replacement is verified by reopening the saved PDF and checking that the replacement text is extractable where possible. Some PDFs can keep stale hidden extraction text even after a visually correct redaction. In that case the endpoint returns HTTP 200 with a warning instead of failing the edit.
