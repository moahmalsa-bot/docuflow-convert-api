...
For AI edit apply operations, use `operation` as the operation-name field. The backend still accepts `type` for older clients.

```javascript
await fetch(`${DOCUFLOW_API_BASE_URL}/pdf/ai-edit/apply`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
