...
For AI edit apply operations, use `operation` as the operation-name field. The backend still accepts `type` for older clients. `replace_text` is a visual edit: the backend covers the selected bounding box with white, inserts replacement text, renders the edited page, and succeeds only if the visible page changed. The response contains a `downloadUrl` for the newly edited PDF version and may include `warnings`; warnings mean the visible edit was applied but the PDF may retain hidden extraction-layer text.
...
AI replace text planning supports instructions like:

```javascript
const planResponse = await fetch(`${DOCUFLOW_API_BASE_URL}/pdf/ai-edit/plan`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    document_id: analysis.document_id,
    instruction: "Change OLD TEXT to NEW TEXT",
    selected_object_ids: [],
    scope: "document",
  }),
});
...
if (plan.requires_confirmation) {
  // Show plan.matches to the user and apply the selected match.operation.
} else {
  // plan is the operations array. Send it to /pdf/ai-edit/apply.
}
```
